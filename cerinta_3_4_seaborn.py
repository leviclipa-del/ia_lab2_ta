...
fig.suptitle('Vizualizarea distribuțiilor cu Seaborn', fontsize=16, fontweight='bold')

# --- Subplot 1: Histogramă cu KDE per specie (Iris) ---
sns.histplot(data=iris, x='petal_length', hue='species', kde=True, bins=15, ax=axes[0, 0])
...
# --- Subplot 3: Violinplot lăţimea petalei per specie (Iris) ---
sns.violinplot(data=iris, x='species', y='petal_width', inner='quart', ax=axes[1, 0])
axes[1, 0].set_title('Violinplot lățimea petalei pe specii')
axes[1, 0].set_xlabel('Specie')
axes[1, 0].set_ylabel('Lățime petală (cm)')

# --- Subplot 4: Boxplot bacşişuri per zi (Tips) ---
sns.boxplot(data=tips, x='day', y='tip', order=['Thur', 'Fri', 'Sat', 'Sun'], ax=axes[1, 1])
axes[1, 1].set_title('Distribuția bacşişurilor per zi (Tips)')
axes[1, 1].set_xlabel('Zi')
axes[1, 1].set_ylabel('Bacşiş ($)')
