...
# --- Subplot 2: Scatter - lungime vs. lăţime petală, colorat per specie ---
culori = {'setosa': '#e74c3c', 'versicolor': '#3498db', 'virginica': '#2ecc71'}
for specie, culoare in culori.items():
    subset = iris[iris['species'] == specie]
    axes[0, 1].scatter(subset['petal_length'], subset['petal_width'], label=specie, color=culoare, alpha=0.7, s=50)
axes[0, 1].set_title('Lungime vs. lăţime petală pe specii')
axes[0, 1].set_xlabel('Lungime petală (cm)')
axes[0, 1].set_ylabel('Lăţime petală (cm)')
axes[0, 1].legend()

# --- Subplot 3: Grafic bare - număr flori per specie ---
numar_per_specie = iris['species'].value_counts()
axes[1, 0].bar(numar_per_specie.index, numar_per_specie.values, color=['#e74c3c', '#3498db', '#2ecc71'], edgecolor='black', alpha=0.8)
...
print("Graficul a fost salvat în fişierul 'grafice_iris_matplotlib.png'.")